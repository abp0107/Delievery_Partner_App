...
class _pancardState extends State<pancard> {
  ...
  Future<void> _loadAadharImages() async {
    ...
    final doc = await FirebaseFirestore.instance
        ...
        .get();
    if (doc.exists) {
      final frontBase64 = doc['PanCardFront'] ?? '';
      final backBase64 = doc['PanCardBack'] ?? '';
      setState(() {
  ...
  Future<void> _submitImages() async {
    try {
      ...
      if (_frontImage != null) {
        ...
        data['PanCardFront'] = base64Encode(compressedFront);
      }

      if (_backImage != null) {
        final compressedBack = await _compressImage(_backImage!);
        data['PanCardBack'] = base64Encode(compressedBack);
      }
      ...
      if (data.isNotEmpty) {
        ...
        await FirebaseFirestore.instance
            ...
            .doc('status')
            .set({'pan': 'pending'}, SetOptions(merge: true));
      }
          ...
          SnackBar(
            content: Row(
              children: const [
                ...
                Expanded(
                  child: Text(
                    "PanCard images uploaded successfully!",
