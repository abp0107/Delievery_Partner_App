...
class AadharCardUploadPage extends StatefulWidget {
  const AadharCardUploadPage({super.key});

  @override
  State<AadharCardUploadPage> createState() => _AadharCardUploadPageState();
}

class _AadharCardUploadPageState extends State<AadharCardUploadPage> {
  File? _frontImage;
  File? _backImage;
  ...
  Future<void> _loadAadharImages() async {
    ...
    final doc = await FirebaseFirestore.instance
        ...
        .get();
    if (doc.exists) {
      final frontBase64 = doc['aadharCardFront'] ?? '';
      final backBase64 = doc['aadharCardBack'] ?? '';
      setState(() {
  ...
  Future<void> _submitImages() async {
    try {
      ...
      if (_frontImage != null) {
        ...
        data['aadharCardFront'] = base64Encode(compressedFront);
      }

      if (_backImage != null) {
        final compressedBack = await _compressImage(_backImage!);
        data['aadharCardBack'] = base64Encode(compressedBack);
      }
      ...
      if (data.isNotEmpty) {
        ...
        await FirebaseFirestore.instance
            ...
            .doc('status')
            .set({'aadhar': 'pending'}, SetOptions(merge: true));
      }
          ...
          SnackBar(
            content: Row(
              children: const [
                ...
                Expanded(
                  child: Text(
                    "Aadhar images uploaded successfully!",
